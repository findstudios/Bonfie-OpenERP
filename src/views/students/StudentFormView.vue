...
async function generateStudentId(): Promise<string> {
  try {
    ...
    return `S${nextNumber.toString().padStart(6, '0')}`
  } catch (error) {
    console.error('生成學生編號失敗:', error)
    // 如果查詢失敗，回退到第一個編號
    return 'S000001'
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    console.error('儲存學生資料失敗:', err)
    error.value = err instanceof Error ? err.message : '儲存學生資料失敗'
  } finally {
    loading.value = false
  }
}
...
async function createStudent() {
  ...
  // 創建學生記錄
  const student = await db.create<Student>('students', studentData)

  // 聯絡人現在由 ContactDataModule 獨立處理，新增模式不需要處理聯絡人
