...
import { generateDirectPDF, generateReceiptPDF, generateDetailedInvoicePDF } from '../directPdfService'
...
describe('directPdfService', () => {
  ...
  describe('generateDirectPDF with invalid payment data', () => {
    it('should handle order with null payment amounts', async () => {
      ...
      await expect(generateDirectPDF(order)).resolves.not.toThrow()
    ...
    it('should handle order with empty payment data', async () => {
      const order: Order = {
        order_id: 'TEST-005',
        original_amount: 1500,
        discount_amount: 0,
      ...
      const order: Order = {
        ...
        payments: [
          ...
          } as any,
          {
            ...
            method: 'credit',
            amount: 1000,
            // Missing receipt_number is optional
          }
        ]
      }
  ...
  describe('error recovery', () => {
    it('should continue PDF generation even when payment validation logs errors', async () => {
      const order: Order = {
        ...
        created_at: new Date().toISOString(),
        payments: [
          null as any, // Null payment object
          undefined as any, // Undefined payment object
          {
            payment_date: '2024-01-01',
            method: 'cash',
            amount: Symbol('test') as any, // Symbol (edge case)
            receipt_number: 'R001'
          }
        ]
      }
